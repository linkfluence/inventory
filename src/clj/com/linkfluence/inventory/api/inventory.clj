...
(defroutes INVENTORY
  ...
  (GET "/alias/:id" [id] (if-let [alias (inventory/get-alias (keyword id))]
                              ...
                              (u/mk-resp 404 "error" {} "Resource not found")))
  (POST "/resource/:id/addtags" [id tags] (if (some? (inventory/get-resource (keyword id) false))
                                            (if (inventory/validate-tags tags)
                                              (put-event {:type "resource" :id (keyword id) :tags tags})
                                              (u/mk-resp 400 "error" {:explanation (inventory/explain-tags tags)} "tag array is invalid"))
                                            (u/mk-resp 404 "error" {} "Cannot add tags to non existent resource")))
  (POST "/alias/:id/addtags" [id tags] (if (some? (inventory/get-alias (keyword id)))
                                          (if (inventory/validate-tags tags)
                                            (put-event {:type "alias" :id (keyword id) :tags tags})
                                            (u/mk-resp 400 "error" {:explanation (inventory/explain-tags tags)} "tag array is invalid"))
                                          (u/mk-resp 404 "error" {} "Cannot add tags to non existent alias")))
  ;;Resource hiding
